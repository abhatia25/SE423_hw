...
uint16_t updown = 0;
uint16_t pr_raw_reading = 0;
float pr_scaled_reading = 0.0;
uint32_t adcaCalls = 0;
uint16_t x_raw_reading = 0;
float x_scaled_reading = 0.0;
uint16_t y_raw_reading = 0;
float y_scaled_reading = 0.0;
...
void main(void)
{
    ...
    {
        if (UARTPrint == 1 ) {
            serial_printf(&SerialA,"Photoresistor Voltage Value:%.3f, Joystick X Voltage Value:%.3f, Joystick Y Voltage Value:%.3f\r\n",pr_scaled_reading,x_scaled_reading,y_scaled_reading);
            UARTPrint = 0;
        }
    }
}
