...
int astar(int rowStart, int colStart, int rowEnd, int colEnd)
{
	...
	while(openSet.numElems > 0 && goalFound != 't')	
	{
		...
		//AB: currDist could be different from the Manhattan distance to the Start position if obstacles are in the way that need to be navigated around. The Manhattan distance assumes a straight line with no obstacles
		...
		for(cnt = 0; cnt<numNeighbors; cnt++)	//for each found neighbor
		{
			...
			//AB: Basically it checks if there's a better node (with a lower distance) than the neighbor node in either the open or closed list and if so, avoids adding the neighbor node to make the algorithm the most efficient
		...
		// IF closedSet
		//AB: push to closedSet because it's been evaluated
